...
In this second example, we're going to be running an **/test** test suite.
Like most default test suites, let's assume ours is stored in the `test/` directory (that's the Rails default too). To run the test suite in `test/`, we might run the following command:
